		...
		if (gvc.canRead()) {
			...
			if (exec != null) {
				...
				button.addActionListener(e -> {
					try {
//						Process process = Runtime.getRuntime().exec(exec, null, new File(exec).getParentFile());
						new ProcessBuilder(exec).inheritIO().directory(new File(exec).getParentFile()).start();
//						new ProcessLogger(LOGGER).start(process);
					} catch (IOException ex) {
						LOGGER.warn("", ex);
					}
				});
	...
	public static PrintStream createLoggingProxy(PrintStream realPrintStream) {
		return new PrintStream(realPrintStream) {
			...
			public void print(String string) {
				...
				LoggerFactory.getLogger("console").info(string);
			}
		};
	}
